...
module.exports = function(controller) {
    ...
    controller.studio.runHandlers = function(type, convo, cb_or_sub, cb) {

        console.log('run handlers for',type,'script', convo.context.script_name);
        if (!convo.context.script_name) {
            return cb(convo);
        } else {
            var handlers = [];
            switch (type) {
                case 'after':
                    handlers = after_hooks[convo.context.script_name] ? after_hooks[convo.context.script_name].slice() : [];
                break;
                case 'before':
                    handlers = before_hooks[convo.context.script_name] ? before_hooks[convo.context.script_name].slice() : [];
                break;
                case 'validate':
                    handlers = (answer_hooks[convo.context.script_name] && answer_hooks[convo.context.script_name][cb_or_sub]) ? answer_hooks[convo.context.script_name][cb_or_sub].slice() : [];
                break;
                case 'beforeThread':
                    handlers = (thread_hooks[convo.context.script_name] && thread_hooks[convo.context.script_name][cb_or_sub]) ? thread_hooks[convo.context.script_name][cb_or_sub].slice() : [];
                break;

            }

            console.log('FOUND HOOKS', handlers.length);

            runHooks(handlers, convo, cb?cb:cb_or_sub);
            ...
            controller.studio.getScript(bot, input_text, user).then(function(command) {
                if (command !== {} && command.id) {
                    ...
                    controller.studio.compileScript(
                        ...
                        // we can run these here since they happen BEFORE the script happens
                        // so any persistence will happen after this.
                        controller.studio.runHandlers('before', convo, function(convo) {
                            resolve(convo);
                        });
